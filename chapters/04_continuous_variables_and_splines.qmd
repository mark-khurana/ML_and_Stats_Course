...
Using the built-in `PBC` dataset (primary biliary cholangitis) from the `survival` package in R, model the relationship between serum bilirubin and transplant-free survival.
...
# Task 1: Fit a Cox model with bilirubin as a linear term
fit_linear <- cph(Surv(time, status == 2) ~ bili, data = pbc)

# Task 2: Fit a Cox model with bilirubin modelled using RCS (4 knots)
fit_rcs <- cph(Surv(time, status == 2) ~ rcs(bili, 4), data = pbc)
...
# Task 6: What does the plot tell you about the bilirubin-survival
...
# Load PBC data (available via lifelines or statsmodels)
from lifelines.datasets import load_rossi
# Note: PBC not in lifelines by default; use a CSV or the R dataset
# For illustration, we simulate similar data:
np.random.seed(42)
n = 300
bili = np.random.exponential(scale=2, size=n)
bili = np.clip(bili, 0.3, 25)
log_hr = 0.1 * bili + 0.005 * bili**2  # Non-linear true relationship
time = np.random.exponential(scale=2000 * np.exp(-log_hr))
event = np.random.binomial(1, 0.7, size=n)

df = pd.DataFrame({'time': time, 'event': event, 'bili': bili})

# Task 1: Fit Cox model with linear bilirubin
cph_linear = CoxPHFitter()
cph_linear.fit(df, duration_col='time', event_col='event',
               formula='bili')
print("Linear model:")
cph_linear.print_summary()

# Task 2: Create spline basis and fit Cox model with splines
spline_basis = dmatrix("cr(bili, df=3) - 1", data=df,
                        return_type='dataframe')
spline_basis.columns = ['spline1', 'spline2', 'spline3']
df_spline = pd.concat([df[['time', 'event']], spline_basis], axis=1)

cph_spline = CoxPHFitter()
cph_spline.fit(df_spline, duration_col='time', event_col='event')
print("\nSpline model:")
cph_spline.print_summary()

# Task 3: Compare AIC
print(f"\nAIC Linear: {cph_linear.AIC_partial_}")
print(f"AIC Spline: {cph_spline.AIC_partial_}")

# Task 4: Plot the predicted log-hazard ratio across bilirubin range
bili_range = np.linspace(df['bili'].min(), df['bili'].max(), 200)
spline_pred = dmatrix("cr(bili_range, df=3) - 1",
                       {"bili_range": bili_range},
                       return_type='dataframe')
spline_pred.columns = ['spline1', 'spline2', 'spline3']

# Calculate log-HR relative to median
log_hr_pred = spline_pred.values @ cph_spline.params_.values
log_hr_pred -= np.median(log_hr_pred)

plt.figure(figsize=(8, 5))
plt.plot(bili_range, log_hr_pred, 'k-', linewidth=2)
plt.xlabel('Serum Bilirubin (mg/dL)')
plt.ylabel('Log Relative Hazard')
plt.title('Non-linear effect of bilirubin on survival (RCS)')
