...
# 3. Plot PPV and NPV across prevalences
prevalences <- seq(0.001, 0.5, by = 0.001)
...
# Calculate PPV and NPV for three populations
...
# Plot PPV and NPV across prevalences
prevalences = np.linspace(0.001, 0.5, 500)
...
for (g in c("A", "B")) {
  idx <- group == g
  roc_g <- roc(true_outcome[idx], pred_prob[idx], quiet = TRUE)
  ci_g <- ci.auc(roc_g)
  cat(sprintf(
    "Group %s: AUC = %.3f (95%% CI: %.3f - %.3f), Prevalence = %.1f%%\n",
    g,
    auc(roc_g),
    ci_g[1],
    ci_g[3],
    mean(true_outcome[idx]) * 100
  ))
}
...
for (g in c("A", "B")) {
  ...
  cat(sprintf(
    "  Group %s: Sens=%.3f  Spec=%.3f  PPV=%.3f\n",
    g,
    sens,
    spec,
    ppv
...
for (g in c("A", "B")) {
  ...
  cat(sprintf(
    "Group %s: %.1f%% predicted positive at threshold %.2f\n",
    g,
    pos_rate * 100,
    threshold
  ))
...
roc_a <- roc(true_outcome[group == "A"], pred_prob[group == "A"], quiet = TRUE)
roc_b <- roc(true_outcome[group == "B"], pred_prob[group == "B"], quiet = TRUE)
...
legend(
  "bottomright",
  legend = c(
    paste("Group A (AUC =", round(auc(roc_a), 3), ")"),
...
for g in ["A", "B"]:
    ...
    print(f"  Group {g}: Sens={sens:.3f}  Spec={spec:.3f}  PPV={ppv:.3f}")
...
for g, color in [("A", "steelblue"), ("B", "darkorange")]:
    mask = group == g
