...
The modern, unified tool in R is **`CMAverse`**, which implements several causal mediation estimators, handles exposure-mediator interaction, and --- valuably --- includes the E-value sensitivity analysis built in.
...
# Mediator (e.g. change in blood pressure): drug lowers it
...
# --- Estimate natural direct/indirect effects with the regression approach ---
res <- cmest(
...
**What the code shows.** We build a cohort where the drug has *two* routes to the outcome: a direct effect (coefficient 0.8) and an indirect one through the mediator (the drug moves the mediator by 2.0, and each mediator unit moves the outcome by 0.6, so the indirect effect is roughly $2.0 \times 0.6 = 1.2$). `cmest()` with `model = "rb"` fits the regression-based estimator: a mediator model and an outcome model, the latter including the exposure-mediator interaction (`EMint = TRUE`). `summary(res)` prints the **natural direct effect** (should land near 0.8), the **natural indirect effect** (near 1.2), the **total effect** (near 2.0, their sum), and the **proportion mediated** (about 1.2/2.0 ~ 60%), each with a bootstrap confidence interval. Read it clinically as: "about 60% of the drug's benefit on the outcome flows through its effect on blood pressure; the remaining 40% is a direct effect by some other mechanism." The final `cmsens()` call performs the E-value-style sensitivity analysis, telling you how strong an *unmeasured* mediator-outcome confounder would have to be to explain away the indirect effect.

An alternative when you want a fast, closed-form regression estimator with explicit interaction handling is **`regmedint`** (the regression-based method of Valeri and VanderWeele), which supports linear, logistic, Poisson, Cox, and accelerated-failure-time outcome models --- useful when your outcome is a binary event or a survival time. The classic **`mediation`** package (Imai and colleagues) is also still widely used and well documented, though `CMAverse` and `regmedint` are the current first choices for causal effect definitions with interaction.
...
a) Use `cmsens()` (R) or reason qualitatively (Python) about how strong an unmeasured mediator-outcome confounder would need to be to explain away the indirect effect.
...
- In R, **`CMAverse`** is the unified first choice (interaction handling + built-in E-value); **`regmedint`** suits binary and survival outcomes; the classic **`mediation`** package remains useful.
...
1. **VanderWeele TJ.** *Explanation in Causal Inference: Methods for Mediation and Interaction.* Oxford University Press, 2015. The definitive modern text on causal mediation.

2. **Valeri L, VanderWeele TJ.** Mediation analysis allowing for exposure-mediator interactions and causal interpretation: theoretical assumptions and implementation with SAS and SPSS macros. *Psychological Methods.* 2013;18(2):137--150. The regression-based estimator behind `regmedint` and `CMAverse`.

3. **Baron RM, Kenny DA.** The moderator-mediator variable distinction in social psychological research. *Journal of Personality and Social Psychology.* 1986;51(6):1173--1182. The historical approach.

4. **Imai K, Keele L, Tingley D.** A general approach to causal mediation analysis. *Psychological Methods.* 2010;15(4):309--334. Basis of the `mediation` R package.

5. **Shi B, Choirat C, Coull BA, VanderWeele TJ, Valeri L.** CMAverse: a suite of functions for reproducible causal mediation analyses. *Epidemiology.* 2021;32(5):e20--e22. [https://bs1125.github.io/CMAverse/](https://bs1125.github.io/CMAverse/).

6. **Yoshida K, et al.** regmedint: regression-based causal mediation analysis with interaction. R package. [https://github.com/kaz-yos/regmedint](https://github.com/kaz-yos/regmedint).

7. **VanderWeele TJ.** Mediation analysis: a practitioner's guide. *Annual Review of Public Health.* 2016;37:17--32 [@vanderweele2016practitioner]. An accessible overview, including the E-value for direct and indirect effects.

8. **Richiardi L, Bellocco R, Zugna D.** Mediation analysis in epidemiology: methods, interpretation and bias. *International Journal of Epidemiology.* 2013;42(5):1511--1519 [@richiardi2013mediation]. A clinical-epidemiology tutorial on the three main biases that undermine the traditional approach.

9. **Hernan MA, Robins JM.** *Causal Inference: What If.* Chapman & Hall/CRC, 2020. For the potential-outcomes foundations underlying nested counterfactuals.
