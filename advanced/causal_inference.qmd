...
**$A$, $Y$ and $L$ --- treatment, outcome, and everything measured beforehand.** These three letters stand in for the three roles every causal question has. $A$ is the treatment or exposure ($A = 1$ treated, $A = 0$ untreated) --- a statin, insulin, an ACE inhibitor. $Y$ is the outcome --- death, HbA1c, acute kidney injury. $L$ is the collection of patient characteristics recorded *before* treatment, such as age, comorbidities and baseline labs; it is a bundle of variables rather than a single one, which is why you will see "adjust for $L$" meaning "adjust for all of them together".
